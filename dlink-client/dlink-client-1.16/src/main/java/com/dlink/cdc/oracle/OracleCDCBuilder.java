...
/**
 * MysqlCDCBuilderflink cdc SQL 开启debezium日志
 *
 * @author wenmo
...
public class OracleCDCBuilder extends AbstractCDCBuilder implements CDCBuilder {
    ...
    public DataStreamSource<String> build(StreamExecutionEnvironment env) {
        ...
        // JsonDebeziumDeserializationSchema 类每个cdc版本都存在, 避免同包路径同类名覆盖问题,
        // 改成每个数据库自己重新实现，默认代码跟JsonDebeziumDeserializationSchema一致
        // sourceBuilder.deserializer(new JsonDebeziumDeserializationSchema());
        sourceBuilder.deserializer(new OracleJsonDebeziumDeserializationSchema());
